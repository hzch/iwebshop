...
abstract class IApplication
{
    ...
    public function __construct($config)
    {
		...
		if($web = true)
		{
			$basePath = rtrim($_SERVER['DOCUMENT_ROOT'].dirname($_SERVER['SCRIPT_NAME']),'\\/').DIRECTORY_SEPARATOR;
	...
	public function getWebRunPath()
	{
		...
		{
			$this->webRunPath = IUrl::creatUrl('').str_replace(array(dirname(realpath(rtrim($_SERVER['DOCUMENT_ROOT'],'/\\')."/".ltrim($_SERVER['SCRIPT_NAME'],'/\\'))).DIRECTORY_SEPARATOR,'\\'),array('','/'),rtrim(realpath($this->getRuntimePath()),'/\\'));
		}
